...
var resetSimulator = function(callback) {
  console.log("APPIUM: resetSimulator");
  if (!rootDriver) {
    callback(true);
    return;
  }
  ...
  rootDriver.run(function* () {
    ...
    while (i < 50) {
      /*try {
        // best-case first
        // reset data
        yield rootDriver.elementById("indexLogin");
        break;
      }
      catch (e) {}*/
      ...
      try {
        // is there an alert?
        yield rootDriver.elementById('OK').click();
      }
      catch (e) { }

      try {
        // is there an alert?
        yield rootDriver.elementById('Allow').click();
      }
      catch (e) { }
    ...
    rootDriver.switchTo().alert().accept();
