...
public class ImageCroppingView extends ImageView {
    ...
    private static final String TAG = "ImageCroppingView";
    ...
    private boolean shouldCallBackAfterInit = false;
    ...
    public void setImageBitmap(Bitmap bm,Matrix m){
        super.setImageBitmap(bm);
        setImageMatrix(m);
    }

    public void shouldCallBackAfterInit(boolean shouldCall) {
        this.shouldCallBackAfterInit = shouldCall;
    }
...
//        canvas.drawRect(0, y3, x, y, paint);
        ...
        final int kWidth = 480;
        final int kHeight = 300;
        //float xH = kWidth / x;
        int zoneHeight = kHeight * (viewWidth / kWidth);

        int hh = viewHeight - 200;
        int mh = 20;

//        Paint paintLine = new Paint();
//        paintLine.setColor(0xFFFFFFFF);
//        paintLine.setStrokeWidth(2);
//        canvas.drawLine(0,(hh - zoneHeight)/2,viewWidth,(hh - zoneHeight)/2,paintLine);
//        canvas.drawLine(2,(hh-zoneHeight)/2,2,(hh-zoneHeight)/2+mh,paintLine);
//        canvas.drawLine(y-2,(hh-zoneHeight)/2,y-2,(hh-zoneHeight)/2+mh,paintLine);
//        canvas.drawLine(0,zoneHeight + (hh-zoneHeight) /2,viewWidth,zoneHeight+(hh-zoneHeight)/2,paintLine);
//        canvas.drawLine(2,zoneHeight+(hh-zoneHeight)/2,2,zoneHeight+(hh-zoneHeight)/2-mh,paintLine);
//        canvas.drawLine(y-2,zoneHeight+(hh-zoneHeight)/2,y-2,zoneHeight+(hh-zoneHeight)/2-mh,paintLine);
...
//                x3,y3,x0,y0},paintLine);
        ...
        if (this.shouldCallBackAfterInit) {
            this.shouldCallBackAfterInit = false;
            dispatchOnChangeImg();
        }
    ...
    public Bitmap getCroppedImage() {
        ...
        Bitmap srcBmp = Bitmap.createBitmap(viewWidth, viewHeight, Bitmap.Config.ARGB_8888);
        draw(new Canvas(srcBmp));
        return srcBmp;
//        if(0 == x0 && 0 == y0) {
//            MsSdkLog.d(TAG, "=============getCroppedImage return srcBmp");
//            return srcBmp;
//        }
//
//        final Bitmap dstBmp = Bitmap.createBitmap(srcBmp,
//                (int) x0,
//                (int) y0,
//                (int) w,
//                (int) w);
...
//        return dstBmp;
    ...
    private void dispatchOnChangeImg () {
        WritableMap event = Arguments.createMap();
        WritableMap data = Arguments.createMap();
        data.putString("uri", "file://" + getCroppedImg());
        event.putMap("data", data);
        ReactContext reactContext = (ReactContext)getContext();
        reactContext.getJSModule(RCTEventEmitter.class).receiveEvent(
                this.viewId > 0 ? this.viewId : getId(),
                "topChange",
                event);
    }

    public String getCroppedImg() {
        Bitmap bitmap = this.getCroppedImage();
        if (bitmap != null) {
            if (filePath == null || filePath.length() == 0) {
                String fileName = FileUtils.fileNameTmpImage();
                filePath = FileUtils.imageStorePath() + fileName;
            } else {
